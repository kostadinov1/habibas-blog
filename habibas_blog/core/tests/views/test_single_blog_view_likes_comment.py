...
class SingleBlogViewCommentLikes(TestCase):
    ...
    # ACTUAL TESTS
    def test_user_can_like_comment_only_once(self):
        ...
        # comment's first like
        comment_like = {'user': user,
                        'comment': comment}
        self.client.post(reverse('like comment', kwargs={'pk': comment.id}),
                                                 data=comment_like)
        # comment's second like
        comment_like_2 = {'user': user,
                          'comment': comment}
        self.client.post(reverse('like comment', kwargs={'pk': comment.id}),
                                                   data=comment_like_2)
        # check
